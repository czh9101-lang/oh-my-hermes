"""Conditional `llm-app-dev` contracts for features that touch live state.

The five build rails and the public-board contract in `render.py` apply to every
LLM feature. The contracts here apply only when a feature presents or acts on
business records, enforces a cumulative business limit, or stores facts about a
person. They live in their own reference so an extraction or RAG handoff does
not carry record, limit, and memory requirements it cannot use.

`LLM_APP_DEV_CONDITIONAL_CONTRACTS` is the activation metadata: the catalog
definition names these triggers in its quality bar so the always-loaded skill
body and this reference agree on when the reference must be loaded. Interpolate
rather than retype.
"""

from __future__ import annotations

# (trigger, contract) pairs. The trigger is the feature property that activates
# the contract; the contract is the section heading it maps to below. A feature
# with none of these properties does not load this reference.
LLM_APP_DEV_CONDITIONAL_CONTRACTS: tuple[tuple[str, str], ...] = (
...
LLM_APP_DEV_STATEFUL_CONTRACTS_REFERENCE_PATH = "references/stateful-contracts.md"

_STATEFUL_CONTRACTS_ATTRIBUTION = """## Attribution
...
def llm_app_stateful_contracts_reference() -> str:
    """Render the conditional record, limit, memory, and preloading contracts."""
    triggers = "\n".join(
        f"- The feature **{trigger}**: load the section on {contract}."
        for trigger, contract in LLM_APP_DEV_CONDITIONAL_CONTRACTS
    )
    return f"""# Stateful Contracts: Records, Limits, Memory
...
{_STATEFUL_CONTRACTS_ATTRIBUTION}
"""
