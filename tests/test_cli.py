...
"""
    ...
    def test_release_product_readiness_cli_summarizes_product_story(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("203/203 route hints aligned", gates["route_hint_alignment"]["summary"])
            ...
            self.assertIn("295/295 interventions", gates["routing_precision"]["summary"])
    ...
    def test_release_evidence_bundle_cli_packages_optional_write_artifact(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("Chat card coverage: 83/83 (generic ack 0)", stdout)
            self.assertIn("Context brief coverage: 11/11 (route hints 10, catalog hints 1)", stdout)
            self.assertIn(
                "Routing precision: 167/167 negative controls, 295/295 interventions "
            ...
            self.assertEqual(payload["summary"]["chat_card_coverage_passing"], 83)
            self.assertEqual(payload["summary"]["chat_card_generic_ack_count"], 0)
            self.assertEqual(payload["summary"]["route_hint_alignment_aligned"], 203)
            ...
            self.assertEqual(payload["summary"]["routing_precision_intervention_passing"], 295)
            self.assertEqual(payload["summary"]["routing_precision_intervention_total"], 295)
    ...
    def test_demo_chat_card_coverage_keeps_user_facing_routes_dedicated(self) -> None:
        ...
        self.assertEqual(payload["summary"]["case_count"], 83)
        self.assertEqual(payload["summary"]["passing_count"], 83)
    ...
    def test_demo_chat_card_coverage_summary_is_human_readable(self) -> None:
        ...
        self.assertIn("Result: 83/83 workflow cards dedicated (all passing)", stdout)
    ...
    def test_demo_route_hint_alignment_keeps_router_and_awareness_hints_in_sync(self) -> None:
        ...
        self.assertEqual(payload["summary"]["case_count"], 203)
        self.assertEqual(payload["summary"]["hinted_count"], 203)
        self.assertEqual(payload["summary"]["aligned_count"], 203)
        ...
        self.assertEqual(explicit_payload["summary"]["case_count"], 203)
    ...
    def test_demo_route_hint_alignment_summary_is_human_readable(self) -> None:
        ...
        self.assertIn("Result: 203/203 route hints aligned (all passing)", stdout)
        self.assertIn("Hints present: 203/203; missing hints: 0; mismatches: 0", stdout)
        self.assertIn(
