...
"""
    ...
    def test_release_product_readiness_cli_summarizes_product_story(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("109/109 negative-control cases", gates["routing_precision"]["summary"])
            self.assertIn("220/220 interventions", gates["routing_precision"]["summary"])
    ...
    def test_release_evidence_bundle_cli_packages_optional_write_artifact(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn(
                "Routing precision: 109/109 negative controls, 220/220 interventions "
            ...
            self.assertEqual(payload["summary"]["routing_precision_passing"], 109)
            self.assertEqual(payload["summary"]["routing_precision_total"], 109)
            self.assertEqual(payload["summary"]["routing_precision_overroute_count"], 0)
            self.assertEqual(payload["summary"]["routing_precision_intervention_passing"], 220)
            self.assertEqual(payload["summary"]["routing_precision_intervention_total"], 220)
