...
class HostCollectorContractTests(unittest.TestCase):
    ...
    def test_actual_redirect_path_is_not_the_requested_navigation_digest(self) -> None:
        ...
        self.assertNotEqual(collector._navigation_digest(requested), collector._navigation_digest(redirected))
        actual = {"origin": "http://127.0.0.1:8123", "navigation_digest": collector._navigation_digest(redirected), "width": 800, "height": 600, "dpr": 1, "engine": "chromium", "version": "152.0.0.0", "locale": "en-US", "timezone": "UTC"}
        self.assertEqual(collector._terminal_blocker({"success": True}, actual, {"origin": actual["origin"], "navigation_digest": collector._navigation_digest(requested)}, {"width": 800, "height": 600, "dpr": 1}, {"engine": "chromium", "version": "152.0.0.0"}, {"locale": "en-US", "timezone": "UTC"}, True), "actual_navigation_digest_mismatch")

    def test_unsupported_engine_and_authenticated_terminal_never_launch(self) -> None:
        item = request("http://127.0.0.1:8123/index.html")
        condition = item["plan"]["condition"]
        browser = condition["browsers"][0]
        cell = item["plan"]["matrix"][0]
        browser["engine"] = "firefox"
        result, capture, commands, session = collector._collect_cell(item, item["plan"], cell, Path(tempfile.mkdtemp()), 1_000_000, 0)
    ...
    def test_worst_case_png_reservation_scales_with_viewport_and_dpr(self) -> None:
        ...
        self.assertGreater(collector._png_worst_case_bytes({"width": 800, "height": 600, "dpr": 2}), collector._png_worst_case_bytes({"width": 800, "height": 600, "dpr": 1}))
    ...
    def test_preexisting_capture_is_not_reserved_or_modified(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIsNone(collector._reserve_capture_path(candidate))
    ...
    def test_malformed_or_missing_network_evidence_is_blocked_not_silently_dropped(self) -> None:
        with self.assertRaisesRegex(collector.CollectorError, "duration"):
            collector._project_network([{"url": "https://example.test/a", "status": 500, "method": "GET"}], {})
    ...
    def test_console_error_messages_are_evidence_and_malformed_a11y_is_rejected(self) -> None:
        evidence = collector._project_console([{"type": "error", "text": "secret"}], [{"message": "uncaught"}])
        self.assertEqual(len(evidence), 2)
        with self.assertRaisesRegex(collector.CollectorError, "accessibility_payload"):
            collector._project_a11y([{"id": "x", "impact": "serious"}])
