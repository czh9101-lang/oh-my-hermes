...
- Every Git argv disables `core.fsmonitor`, disables optional locks, strips ambient `GIT_DIR`, `GIT_WORK_TREE`, `GIT_INDEX_FILE`, and `GIT_CONFIG_*` redirection, and uses no diff or textconv command. Configured filters are rejected by a bounded attribute check before `git status` can refresh the temporary index. A configured content filter, sparse checkout, split index, assume-unchanged/skip-worktree entry, gitlink, nested repository, non-SHA-1 Git object format, observed race, unreadable path, timeout, or unavailable repository is an explicit non-authoritative state.
